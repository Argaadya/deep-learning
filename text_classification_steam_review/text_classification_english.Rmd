...
train_y <- to_categorical(data_train$user_suggestion)
```
...
model <- keras_model_sequential(name = "lstm_model") %>% 
  ...
  layer_lstm(name = "LSTM",
             units = 8,
             kernel_regularizer = regularizer_l1_l2(l1 = 0.075, l2 = 0.075),
             return_sequences = F
             ) %>% 
...
The model will be trained using `adam` optimizer with learning rate of 0.001 with `binary cross-entropy` loss function. The model trained with 10 epoch and 64 batch size. We will also use 10% of the training dataset as the validation dataset to monitor if the model will go overfit after some time.
...
```{r}
data.frame(Accuracy = accuracy_vec(pred_class, true_class),
           Recall = sens_vec(pred_class, true_class),
           Precision = precision_vec(pred_class, true_class),
           "F1 Score" = f_meas_vec(pred_class, true_class),
           check.names = F
           )
```
